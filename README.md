...
xz-dev Pi is distributed through immutable [GitHub Releases](https://github.com/xz-dev/pi/releases). Each Release ships all 12 official Bun compile targets supported by Pi's native dependencies: Darwin x64 baseline/modern and arm64; Linux GNU and musl x64 baseline/modern and arm64; and Windows x64 baseline/modern and arm64. Bundle names identify OS, architecture, libc ABI where applicable, and x64 CPU variant. The POSIX installer detects GNU libc versus musl and AVX2 versus baseline; the Windows installer detects architecture and uses AVX2 when Windows reports it, otherwise falling back to baseline. Tests and managed deployments may override selection with `PI_XZ_TARGET_CPU`, `PI_XZ_TARGET_LIBC`, and `PI_XZ_TARGET_ARCH`. A standalone install requires no Node.js, Bun, npm, or package manager at runtime; the executable and its adjacent native modules, WASM, themes, assets, HTML export templates, docs, and examples travel together as one version-matched archive. Installation keeps only the current, previous, and staging bundles needed for safe switching and Windows cleanup.
