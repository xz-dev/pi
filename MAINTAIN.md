...
- Native self-update and `pi update --clean` share the install-root `update.lock`. The lock is intentionally never reclaimed by elapsed time alone. After a crash, first confirm that no Pi self-update or cleanup process is still running and that no operator is using the installation; only then remove the orphaned `update.lock` directory manually and retry. Never delete that lock while a live owner may exist.
