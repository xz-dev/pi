...
- Integrate `patch/session-tree-splice` as its descendant range after `patch/slow-hook-tui-only`, then integrate `patch/provider-transparent-compaction` as the descendant range after session-tree splice. Compaction must preserve slow-hook non-persistence, raw extension-producer freshness, filtered Pi-producer freshness, current-lineage/generation/model/thinking fences, persistence-before-publication, and bounded classic fallback.
