...
- Integrate `patch/session-tree-splice` as its descendant range after `patch/slow-hook-tui-only`, then integrate `patch/provider-transparent-compaction` after session-tree splice and `patch/pre-provider-compaction` after provider-transparent compaction. The final dependent patch is applied only as `patch/provider-transparent-compaction..patch/pre-provider-compaction`. Compaction must preserve slow-hook non-persistence, raw extension-producer freshness, filtered Pi-producer freshness, current-lineage/generation/model/thinking fences, persistence-before-publication, bounded classic fallback, pre-provider active-context replacement, one tool execution, and one committed boundary.
