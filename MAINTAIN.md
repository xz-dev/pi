...
- `pi update --self` from an xz-dev binary discovers only `xz-dev/pi`, downloads the exact target recorded in `piConfig.releaseTarget`, verifies manifest/checksum/archive identity, and activates it without a script installer. An xz-dev source checkout remains user-managed.
