...
- `scripts/lib/bun-targets.mjs` is the authoritative 12-target Bun descriptor matrix. `scripts/build-binaries.sh` produces version-correct target bundles with downstream metadata (`piConfig.distribution=xz-dev`, downstream distribution version), matching GNU/musl native dependencies, and every runtime asset. `scripts/prepare-github-release.mjs` builds locally or aggregates matrix artifacts, verifies each archive's machine-checkable required-path inventory and archive safety, and emits `release-manifest.json`, standalone generated `install.sh` / `install.ps1` installers, `SHA256SUMS`, and `attestation-subjects.jsonl`. There is no hybrid npm tarball.
...
- The pre-push workflow must prepare and install the exact candidate before updating `main`. The publication workflow generates its build matrix from the authoritative descriptors, builds one target per parallel job, aggregates exactly 12 archives, verifies all inventories, and runs native smoke only where GitHub-hosted runners exist. Current native gaps are Linux musl (x64/arm64) and Windows arm64; these bundles receive archive/inventory verification but are not represented as native-validated. Darwin x64 is explicitly exercised on Intel macOS runners.
- The publication job alone receives `contents:write`, `id-token:write`, and `attestations:write`. It creates GitHub build-provenance attestations for the 12 bundles, manifest, installers, and checksum file, then verifies those stored attestations before making a Release public. `attestation-subjects.jsonl` is an immutable Release audit asset but is not itself an attestation subject.
