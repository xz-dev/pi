...
function record(target, archive = { sha256: digest, bytes: 1 }) {
	return { schemaVersion: 1, target: target.id, archive: { file: binaryArchiveName(target.id), sha256: archive.sha256, bytes: archive.bytes, extractedBytes: 1 }, runner: { os: target.runnerOs, arch: target.runnerArch, osArchitecture: target.arch, cpuFeatures: target.requiredCpuFeatures.join(" ") }, executor: { kind: target.executor, containerDigest: target.containerImage ?? null, emulated: false }, commands: target.requiredCommands.map((name) => ({ name, status: 0, elapsedMs: 1 })), tui: { harness: target.os === "windows" ? "Win32 ConPTY API" : "Bun.Terminal PTY", elapsedMs: 1, outputBytes: 1, input: target.os === "windows" ? "/exit\\r" : "ctrl-c,ctrl-d", childExitCode: 0, observedOutput: true, exitSent: true, cleanExit: true }, clipboard: { loadedAndCalled: true }, thirdPartyNotices: { file: "THIRD_PARTY_NOTICES.md", sha256: createHash("sha256").update(notice).digest("hex"), bytes: Buffer.byteLength(notice) }, timingsMs: { version: 1, help: 1, listModels: 1, interactive: 1 }, limits: SMOKE_LIMITS };
}
function fixture() {
	...
	for (const target of BUN_TARGETS) {
		const stage = join(root, `stage-${target.id}`); mkdirSync(target.os === "windows" ? stage : join(stage, "pi"), { recursive: true });
		writeFileSync(join(stage, target.os === "windows" ? "THIRD_PARTY_NOTICES.md" : "pi/THIRD_PARTY_NOTICES.md"), notice);
		const archivePath = join(root, binaryArchiveName(target.id));
		if (target.os === "windows") execFileSync("zip", ["-q", archivePath, "THIRD_PARTY_NOTICES.md"], { cwd: stage });
		else execFileSync("tar", ["-czf", archivePath, "-C", stage, "pi"]);
		const identity = { sha256: createHash("sha256").update(readFileSync(archivePath)).digest("hex"), bytes: statSync(archivePath).size };
...
test("aggregator accepts exact authoritative target descriptors", () => {
	const value = fixture();
	try { execFileSync(process.execPath, [join(import.meta.dirname, "aggregate-binary-acceptance.mjs"), value.records, value.manifest, value.output]); }
...
test("aggregator rejects self-asserted emulation and runner mismatch", () => {
	const value = fixture();
	try {
		const path = join(value.records, `${BUN_TARGETS[0].id}.json`); const changed = JSON.parse(readFileSync(path, "utf8")); changed.executor.emulated = true; writeFileSync(path, JSON.stringify(changed));
...
test("aggregator rejects TUI evidence that does not match the target platform", () => {
	...
	try {
		...
		const path = join(value.records, `${target.id}.json`); const changed = JSON.parse(readFileSync(path, "utf8")); changed.tui.harness = "Python standard-library PTY"; changed.tui.input = "/exit\\r"; writeFileSync(path, JSON.stringify(changed));
		const result = spawnSync(process.execPath, [join(import.meta.dirname, "aggregate-binary-acceptance.mjs"), value.records, value.manifest, value.output], { encoding: "utf8" });
		assert.notEqual(result.status, 0);
...
test("aggregator rejects notice evidence that does not match archive bytes", () => {
	const value = fixture();
	try {
		const path = join(value.records, `${BUN_TARGETS[0].id}.json`); const changed = JSON.parse(readFileSync(path, "utf8")); changed.thirdPartyNotices.sha256 = digest; writeFileSync(path, JSON.stringify(changed));
		const result = spawnSync(process.execPath, [join(import.meta.dirname, "aggregate-binary-acceptance.mjs"), value.records, value.manifest, value.output], { encoding: "utf8" });
		assert.notEqual(result.status, 0);
