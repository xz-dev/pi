...
function record(target, archive = { sha256: digest, bytes: 1 }) {
	return { schemaVersion: 1, target: target.id, archive: { file: binaryArchiveName(target.id), sha256: archive.sha256, bytes: archive.bytes, extractedBytes: 1 }, runner: { os: target.runnerOs, arch: target.runnerArch, osArchitecture: target.arch, cpuFeatures: target.requiredCpuFeatures.join(" ") }, executor: { kind: target.executor, containerDigest: target.containerImage ?? null, emulated: false }, commands: target.requiredCommands.map((name) => ({ name, status: 0, elapsedMs: 1 })), tui: { harness: target.os === "windows" ? "Win32 ConPTY API" : "Python standard-library PTY", elapsedMs: 1, outputBytes: 1, input: "/exit\\r", childExitCode: 0, observedOutput: true, exitSent: true, cleanExit: true }, clipboard: { loadedAndCalled: true }, thirdPartyNotices: { file: "THIRD_PARTY_NOTICES.md", sha256: createHash("sha256").update(notice).digest("hex"), bytes: Buffer.byteLength(notice) }, timingsMs: { version: 1, help: 1, listModels: 1, interactive: 1 }, limits: SMOKE_LIMITS };
}
function fixture() {
