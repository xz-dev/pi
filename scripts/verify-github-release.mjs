...
function assertManifest(manifest, requireFullSet) {
	const allowedKeys = new Set(["schemaVersion", "repository", "tag", "distributionVersion", "apiVersion", "commit", "packaging", "layoutVersion", "bundles", "requiredPaths", "installer", "attestation"]);
	...
	if (requireFullSet && JSON.stringify(platforms) !== JSON.stringify([...BINARY_PLATFORMS].sort())) throw new Error("Manifest bundles must cover exactly the six canonical platforms");
...
const assets = [...Object.values(manifest.bundles).map((bundle) => bundle.file), MANIFEST_FILENAME, INSTALL_SH_FILENAME, INSTALL_PS1_FILENAME, SUMS_FILENAME];
...
for (const file of [MANIFEST_FILENAME, INSTALL_SH_FILENAME, INSTALL_PS1_FILENAME]) assertAsset(releaseDir, sums, file);
if (sums.size !== assets.length - 1) throw new Error("SHA256SUMS must contain the six bundles, manifest, and both installers only");
