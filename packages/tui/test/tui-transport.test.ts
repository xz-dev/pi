...
describe("TUI terminal transport behavior", () => {
	...
	it("releases frequent centered Fleet updates with the editor cursor in the synchronized frame", async (t) => {
		const terminal = new LoggingVirtualTerminal(209, 50);
		const tui = new TuiMainScreen(terminal, true);
		t.after(() => tui.stop());
		const base = new StaticLines(["x".repeat(209), ...Array.from({ length: 54 }, (_, index) => `base-${index}`)]);
		const editor = new Editor(tui, defaultEditorTheme);
		editor.setText("draft");
		...
		for (let frame = 1; frame <= 4; frame++) {
			resetOutputObservations(terminal);
			fleet.frame = frame;
			base.lines[1] = `base update ${frame}`;
			tui.requestRender();
			await terminal.waitForRender();

			const titles = terminal.getViewport().filter((line) => line.includes("Subagent fleet inspector"));
			assert.strictEqual(titles.length, 1, `Fleet frame ${frame} left a duplicate title`);
			assert.strictEqual(
				terminal.getAcceptedWriteCount(),
				1,
				`Fleet frame ${frame} emitted cursor state after the synchronized release`,
			);
			assert.ok(
				terminal.getCursorVisibilityTransitions().every((transition) => transition.synchronizedOutput),
				`Fleet frame ${frame} changed cursor visibility outside synchronized output`,
			);
		}
	});
	...
	it("recovers after a synchronized frame is only partially delivered", async (t) => {
		...
		const oracleTui = new TuiMainScreen(oracleTerminal, true);
