...
describe("openai-responses provider defaults", () => {
	...
	it("accepts compact usage without output token details", async () => {
		const result = await compactOpenAIResponses(
			...
			{ messages: [{ role: "user", content: "old", timestamp: 1 }] },
			{
				...
				fetch: async () =>
					new Response(
						JSON.stringify({
							id: "resp_compact_compat",
							created_at: 1,
							object: "response.compaction",
							output: [{ type: "compaction", id: "cmp_compat", encrypted_content: "opaque" }],
							usage: {
								input_tokens: 2,
		...
		expect(result.usage.reasoning).toBe(0);
	});
	...
	it("keeps prior opaque compact output out of onPayload and retained hook objects", async () => {
		const seen: unknown[] = [];
		const retained: unknown[] = [];
		const compactOutput = [{ type: "compaction", id: "cmp_priv", encrypted_content: "opaque-private-sentinel" }];
		vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
			const url = input instanceof Request ? input.url : String(input);
			if (url.endsWith("/responses/compact")) {
				return new Response(
					JSON.stringify({
						id: "resp_compact_priv",
						created_at: 1,
						object: "response.compaction",
						output: compactOutput,
						usage: {
		...
		const model = getModel("openai", "gpt-5.4");
		const first = await compactOpenAIResponses(
			model,
			{ messages: [{ role: "user", content: "old", timestamp: 1 }] },
			{ apiKey: "test-key" },
		);
		await compactOpenAIResponses(
			model,
			{ messages: [{ role: "user", content: "later", timestamp: 2 }] },
			{
				apiKey: "test-key",
				previous: first,
				onPayload: (payload) => {
		...
		await stream.result();
		expect(JSON.stringify(seen)).not.toContain("opaque-private-sentinel");
		expect(JSON.stringify(retained)).not.toContain("opaque-private-sentinel");
	});
