...
function getMessageFromEntryForCompaction(entry: SessionEntry): AgentMessage | undefined {
	...
	return message?.role === "bashExecution" && message.excludeFromContext ? undefined : message;
}

function filteredCompactionEntries(pathEntries: SessionEntry[]): SessionEntry[] {
	return pathEntries.filter(isCompactionDependencyEntry);
}
...
export function prepareCompaction(
	unfilteredPathEntries: SessionEntry[],
	settings: CompactionSettings,
): CompactionPreparation | undefined {
	const pathEntries = filteredCompactionEntries(unfilteredPathEntries);
...
export async function compactRawHistoryInBounds(
	...
	const raw = filteredCompactionEntries(pathEntries).filter((entry) => entry.type !== "compaction");
