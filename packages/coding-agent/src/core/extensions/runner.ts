...
export class ExtensionRunner {
	...
	constructor(
		...
		getSlowHookThresholdMs: () => number = () => 0,
	) {
	...
	private async runHandler<T>(
		...
		try {
			...
			const elapsedMs = performance.now() - startedAt;
			try {
				const threshold = this.getSlowHookThresholdMs();
				let slow = false;
				try {
					slow = threshold !== 0 && elapsedMs > threshold;
				} catch {
					// Diagnostics must never alter extension behavior.
				}
				if (isShutdown) {
	...
	emitUninterruptibleMessageEnd(event: MessageEndEvent): AgentMessage | undefined {
		const ctx = this.createContext();
