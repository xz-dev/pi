...
function compactExtensionIdentity(extensionPath: string): string {
	...
	const withoutStringControls = extensionPath.replace(
		/(?:\x1b[\x50\x58\x5d\x5e\x5f]|\x90|\x98|\x9d|\x9e|\x9f)[\s\S]*?(?:\x07|\x1b\\|\x9c|$)/g,
		"",
	);
	const sanitized = stripAnsi(withoutStringControls)
