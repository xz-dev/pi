...
import {
	...
	SLOW_EXTENSION_HOOK_ENTRY_TYPE,
...
describe("ExtensionRunner shutdown progress", () => {
	...
	it("emits serial handler progress, keeps JSONL, and writes no session or model context when a listener is attached", async () => {
		...
		const logText = fs.readFileSync(path.join(tempDir, "logs", "extension-lifecycle.jsonl"), "utf8");
		...
		expect(entries.map((entry) => entry.status)).toEqual(["start", "end", "start", "error"]);
	});

	it("keeps baseline session diagnostics when no interactive shutdown listener is attached", async () => {
		const { extensionPath, result, runner, sessionManager, progress } = await loadShutdownRunner();
		let now = 1_000;
		vi.spyOn(performance, "now").mockImplementation(() => now);
		result.extensions[0].handlers.set("session_shutdown", [
		...
		expect(progress).toEqual([]);
		const custom = sessionManager.getEntries().filter((entry) => entry.type === "custom");
		expect(custom).toEqual([
			expect.objectContaining({
				type: "custom",
				customType: SLOW_EXTENSION_HOOK_ENTRY_TYPE,
