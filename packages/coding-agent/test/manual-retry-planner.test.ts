...
describe("planContinuation", () => {
	test.each(["error", "aborted"] as const)(
		...
		(stopReason) => {
			...
			const plan = planContinuation({ branchEntries: branch });
	...
	test("classifies an explicit user anchor without duplicating context", () => {
		...
		const plan = planContinuation({ branchEntries: branch, selectedEntryId: "user-1" });
	...
	test("plans from a selected tool-call assistant or toolResult protocol node", () => {
		...
		const fromCall = planContinuation({ branchEntries: branch, selectedEntryId: "assistant-1" });
		expect(fromCall.kind).toBe("tool_batch");
		expect(fromCall.anchorEntryId).toBe("result-1");
		expect(fromCall.recoveryMessages).toEqual([]);

		const fromResult = planContinuation({ branchEntries: branch, selectedEntryId: "result-1" });
	...
	test("rejects a completed assistant with nothing_to_continue", () => {
		...
		expectRejection("nothing_to_continue", () => planContinuation({ branchEntries: branch }));
	});
	...
	test("rejects an orphan tool result", () => {
		...
		expectRejection("orphan_tool_result", () => planContinuation({ branchEntries: branch }));
	});
	...
	test("rejects duplicate tool call ids in the assistant batch", () => {
		...
		expectRejection("duplicate_tool_call", () => planContinuation({ branchEntries: branch }));
	});
	...
	test("rejects duplicate tool results for the same call id", () => {
		...
		expectRejection("duplicate_tool_result", () => planContinuation({ branchEntries: branch }));
	});
	...
	test("rejects tool name mismatch between call and result", () => {
		...
		expectRejection("tool_name_mismatch", () => planContinuation({ branchEntries: branch }));
	});

	test("rejects interleaved non-toolResult content inside a tool batch", () => {
		const branch = [
			userMessage("user-1", null, "bad"),
			assistantEntry(
				"assistant-1",
			...
			toolResultEntry("result-1", "assistant-1", "call-1", "first", "kept"),
			userMessage("user-2", "result-1", "interleaved"),
			toolResultEntry("result-2", "user-2", "call-2", "second", "late"),
		];
		// Whole-batch validation from the tool-call assistant sees the user before call-2 resolves.
		expectRejection("interleaved_tool_batch", () =>
			planContinuation({ branchEntries: branch, selectedEntryId: "assistant-1" }),
		);
	});
	...
	test("rejects malformed empty tool call ids", () => {
		...
		expectRejection("malformed_tool_call", () => planContinuation({ branchEntries: branch }));
	});
	...
	test("rejects non-message selected anchors with nothing_to_continue", () => {
		...
		expectRejection("nothing_to_continue", () =>
			planContinuation({ branchEntries: branch, selectedEntryId: "model-1" }),
		);
	});
