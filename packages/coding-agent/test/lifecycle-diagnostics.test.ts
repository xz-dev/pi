...
describe("transient extension timing diagnostics", () => {
	...
	it("times context and context_with_system handlers with stable indices and restored system messages", async () => {
		const { runner } = await createRunner(`export default function(pi) {
			pi.on("context", async (event) => {
				if (event.messages.some((message) => message.role === "system")) throw new Error("system message leaked");
				event.messages.push({ role: "user", content: [{ type: "text", text: "injected" }], timestamp: 0 });
			});
			pi.on("context_with_system", async (event) => {
				if (event.messages[0]?.role !== "system") throw new Error("system message hidden from opt-in handler");
		...
		const userMessage: UserMessage = { role: "user", content: [{ type: "text", text: "hi" }], timestamp: 1 };
		const result = await runner.emitContext([systemMessage, userMessage]);

		// context handlers never see the system message; the emitted transcript keeps it leading.
		expect(result.map((message) => message.role)).toEqual(["system", "user", "user"]);
		expect(errors).toEqual([]);
		expect(notices).toEqual([
			expect.objectContaining({ event: "context", handlerIndex: 0, executionKind: "async" }),
			expect.objectContaining({ event: "context_with_system", handlerIndex: 0, executionKind: "async" }),
		]);
	});

	it("times each context handler at its own index", async () => {
		const { runner } = await createRunner(`export default function(pi) {
			pi.on("context", async () => {});
			pi.on("context", async () => {});
		}`);
		...
		expect(notices.map((entry) => entry.handlerIndex)).toEqual([0, 1]);
	});
