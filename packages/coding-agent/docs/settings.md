...
| `slowHookThresholdMs` | number | `-1` | In interactive TUI, show a transient reminder for each registered extension hook taking longer than this many milliseconds. Negative values disable the reminder entirely; `0` reminds on every hook. Timing diagnostics are not persisted. |
