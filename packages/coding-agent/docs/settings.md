...
| `compaction.remoteEnabled` | boolean | `true` | Use provider-native compaction when supported, falling back to local compaction |
...
| `slowHookThresholdMs` | number | `100` | Show a transcript reminder for each registered extension hook taking longer than this many milliseconds. |
