...
async function emitFinalized(emit: AgentEventSink, event: AgentEvent, signal?: AbortSignal): Promise<void> {
	try {
		// Invoke first so reducer-side finalization still runs when the signal is already aborted.
		await abortable(emit(event), signal);
	} catch (error) {
...
async function finishTurn(
	...
	try {
		// Unlike callAbortable(), this must invoke finishTurn even when finalization observes an aborted signal.
		return (await abortable(config.finishTurn?.(turn, signal), signal)) ?? undefined;
	} catch (error) {
		if (!signal?.aborted) throw error;
		return undefined;
	}
	...
	while (true) {
		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			...
			if (lastCompletedTurn) {
				...
				throwIfAborted(signal);
				const nextTurnSnapshot = await callAbortable(() => config.prepareNextTurn?.(completedTurn), signal);
				if (nextTurnSnapshot) {
			...
			const requestUpdate = await callAbortable(
				() =>
			...
			if (message.stopReason === "error" || message.stopReason === "aborted" || signal?.aborted) {
				lastCompletedTurn = {
					message,
					toolResults: [],
					context: currentContext,
				...
				const completedTurn = lastCompletedTurn;
				await finishTurn(config, completedTurn, signal);
				throwIfAborted(signal);
				await emitFinalized(emit, { type: "turn_end", message, toolResults: [] }, signal);
				throwIfAborted(signal);
				await emitFinalized(emit, { type: "agent_end", messages: newMessages }, signal);
				return;
			}
			...
			const completedTurn = lastCompletedTurn;
			const decision = await finishTurn(config, completedTurn, signal);
			throwIfAborted(signal);
			await emitFinalized(emit, { type: "turn_end", message, toolResults }, signal);
			throwIfAborted(signal);
	...
	while (true) {
		...
		switch (event.type) {
			...
			case "error": {
				...
				if (!addedPartial) {
					await emitFinalized(emit, { type: "message_start", message: { ...finalMessage } }, signal);
				}
				await emitFinalized(emit, { type: "message_end", message: finalMessage }, signal);
				return finalMessage;
			}
		}
	}
...
async function awaitPreparedToolExecution(
	...
	const taskId = config.managedExecutions.register({
		...
		controller: execution.toolController,
		completion,
	});
...
async function executePreparedToolCall(
	...
	try {
		const result = await callAbortable(
			() =>
				prepared.tool.execute(prepared.toolCall.id, prepared.args as never, toolSignal, (partialResult) => {
...
async function emitToolExecutionEnd(
	...
	await emitFinalized(emit, event, signal);
}
...
async function emitToolResultMessage(
	...
	await emitFinalized(emit, { type: "message_start", message: toolResultMessage }, signal);
	await emitFinalized(emit, { type: "message_end", message: toolResultMessage }, signal);
}
